...
class SetSummary {
  ...
  SetSummary copyWith({
    ...
    int? bestRepNumber,
    int? worstRepNumber,
    double? velocityLossPercent,
    double? repVariabilitySD,
    double? fatigueIndex,
    double? meanROM,
    double? meanAvgZAccel,
    double? meanPeakZAccel,
    DateTime? timestamp,
    String? exercise,
    double? loadLbs,
    double? estimatedMeanPower,
    double? estimatedPeakPower,
    double? estimated1RMLbs,
    double? impulse,
    String? suggestedNextLoad,
  }) {
    return SetSummary(
      reps: reps ?? this.reps,
      totalReps: totalReps ?? this.totalReps,
      setDuration: setDuration ?? this.setDuration,
      totalTUT: totalTUT ?? this.totalTUT,
      meanMCV: meanMCV ?? this.meanMCV,
      meanPCV: meanPCV ?? this.meanPCV,
      bestMCV: bestMCV ?? this.bestMCV,
      worstMCV: worstMCV ?? this.worstMCV,
      bestRepNumber: bestRepNumber ?? this.bestRepNumber,
      worstRepNumber: worstRepNumber ?? this.worstRepNumber,
      velocityLossPercent: velocityLossPercent ?? this.velocityLossPercent,
      repVariabilitySD: repVariabilitySD ?? this.repVariabilitySD,
      fatigueIndex: fatigueIndex ?? this.fatigueIndex,
      meanROM: meanROM ?? this.meanROM,
      meanAvgZAccel: meanAvgZAccel ?? this.meanAvgZAccel,
      meanPeakZAccel: meanPeakZAccel ?? this.meanPeakZAccel,
      timestamp: timestamp ?? this.timestamp,
      exercise: exercise ?? this.exercise,
      loadLbs: loadLbs ?? this.loadLbs,
      estimatedMeanPower: estimatedMeanPower ?? this.estimatedMeanPower,
      estimatedPeakPower: estimatedPeakPower ?? this.estimatedPeakPower,
      estimated1RMLbs: estimated1RMLbs ?? this.estimated1RMLbs,
      impulse: impulse ?? this.impulse,
      suggestedNextLoad: suggestedNextLoad ?? this.suggestedNextLoad,
    );
  }
